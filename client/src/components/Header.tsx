...
import { useVisibleMenuStore, useUserStore, useBasketStore } from "../store";
import logo from "../assets/images/logo.png";
import Tippy from "@tippyjs/react";
import { getBasket, getBasketProduct, getProducts } from "../http/basket-api";
...
export default function Header() {
	...
	const { setItems: setBasketItems } = useBasketStore();
	const { isAuth: userIsAuth, exit: userExit } = useUserStore();

	async function fetchBasketItems() {
		await getBasket({ userId: jwtDecode(String(localStorage.getItem("token"))).id }).then((data) => {
			getBasketProduct({ basketId: data.id }).then((basket) => {
				getProducts({ productId: basket.id }).then((data) => setBasketItems(data));
			});
		});
	}
